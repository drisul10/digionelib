                ...
                <table class="table " id="mytable">
                    ...
                    <tbody>
                        <?php
                            ...
                            $db= new database();
                            $x=1;
                            foreach ($db->getDataPengembalianUser() as $key => $value) {
                        ?>
                            <tr>
                                ...
                                <td><?=date('H:i:s, d F Y',strtotime($value->tanggal_pengembalian))?></td>
                                <td>Rp. <?=number_format($value->denda,0,',','.')?></td>
                                <td><?php
                                    if ($value->status_pengembalian==0){
                                        echo '<label class="label label-danger">Belum</label>';
                                    }else {
                                        echo '<label class="label label-success">'.date('H:i:s, d F Y',strtotime($value->pengembalian)).'</label>';
                                    }
                                ?></td>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                $(\'#mytable tbody\').on( \'click\', \'button\', function () {
                    ...
                    $.ajax({
                        url : "'.$base_url.'api.php?get=getDetailPengembalianUser&id="+data[0],
                        type: "GET",
